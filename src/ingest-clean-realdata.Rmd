...
stillbirth_Classify <- full_dat %>%
  ...
  filter(Age >= 18) %>%
  select(-Stillborn, -Preterm, -Gonorrhea, -Syphilis, -Chlamydia, -HepatitisB, -HepatitisC, -MaritalStatus, -PrevPretermBirths, -ART, -InfertilityDrugs) %>% 
  na.omit() %>% 
  ...
  tidyr::spread(key = Education, value = education_hot, fill = 0) # 13 while preserving factors also

stillbirth_Classify <- select(stillbirth_Classify, -id, -Version, -DeliveryYear)
...
preterm_classify <- full_dat %>%
  filter(!(Preterm < 21 & Stillborn == 0)) %>%
  filter(Age >= 18) %>%
  select(-Stillborn, -Preterm, -ART, -InfertilityDrugs) %>%
  na.omit() %>%
  dplyr::mutate(id = dplyr::row_number()) %>%
  mutate(BMI = normalize(BMI)) %>%
  mutate(Height = normalize(Height)) %>%
...
preterm_Classify <- select(preterm_Classify, -id, -Version, -DeliveryYear)
