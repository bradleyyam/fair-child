...
fd_2014_clean <- fd_2014[fd_selected_columns] %>%
  mutate(Stillborn = 1) %>%
  mutate(Pretern = 0) %>%
  set_colnames(fd_renamed_columns)
...
fd_2015_clean <- fd_2015[fd_selected_columns] %>%
  mutate(Stillborn = 1) %>%
  mutate(Pretern = 0) %>%
  set_colnames(fd_renamed_columns)
...
fd_2016_clean <- fd_2016[fd_selected_columns] %>%
  mutate(Stillborn = 1) %>%
  mutate(Preterm = 0) %>%
  set_colnames(fd_renamed_columns)
...
setwd("~/Desktop/fair-child/data/clean/fd")
fd_clean <- ldply(list.files(), read.csv, header=TRUE)
fwrite(fd_clean, "../fd_clean.csv")
# add columns to match with natality data
fd_clean <- mutate(fd_clean, Gonorrhea = NA, Syphilis = NA, Chlamydia = NA, HepatitisB = NA, HepatitisC = NA, .after = PrevCesareans) %>%
  mutate(MaritalStatus = NA, .after = Race) %>%
  mutate(PrevPretermBirths = NA, .after = HypertensionEclampsia)
...
natl2014 <- fread("../data/cdc/natl/natl2014.csv") 

natl2014_clean <- natl2014 %>% 
  mutate(Stillborn = 0) %>%
  mutate(revision = 0) %>%
  select(revision, dob_yy, mager, mbrace, dmar, meduc, priorterm, wic, cig_0, bmi, m_ht_in, pwgt_r, lbo_rec, rf_diab, rf_gest, rf_phyp, rf_ghyp, rf_eclam, rf_ppterm, rf_inftr, rf_fedrg, rf_artec, rf_cesar, ip_gono, ip_syph, ip_chlam, ip_hepatb, ip_hepatc, Stillborn, gestrec3) %>%
  set_colnames(renamed_columns)
...
natl2015 <- fread("../data/cdc/natl/natl2015.csv") 
...
natl2016 <- fread("../data/cdc/natl/natl2016.csv") 
...
setwd("~/Desktop/fair-child/data/clean/natl")
